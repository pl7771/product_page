...
const QRCode = () => (
  <svg viewBox="0 0 100 100" className="w-full h-full text-black bg-white p-2 rounded-lg" fill="currentColor">
    <path d="M10,10 h30 v30 h-30 z M15,15 v20 h20 v-20 z" />
    <path d="M60,10 h30 v30 h-30 z M65,15 v20 h20 v-20 z" />
    <path d="M10,60 h30 v30 h-30 z M15,65 v20 h20 v-20 z" />
      ...
      <nav className={`fixed w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-green-950/80 backdrop-blur-xl border-b border-white/5 py-4' : 'bg-transparent py-6'}`}>
        <div className="max-w-7xl mx-auto px-6 md:px-12 flex justify-between items-center">
          ...
          <div className="hidden md:flex items-center gap-8">
            ...
            <a href="#contact" className="px-6 py-2 bg-white text-black font-semibold text-sm rounded-full hover:bg-green-200 transition-colors">
              Contact Sales
            </a>
          </div>
        ...
        <div className="relative z-20 max-w-5xl mx-auto px-6 text-center mt-20">
          ...
          <Reveal delay={200}>
            <h1 className="text-5xl md:text-7xl lg:text-8xl font-black tracking-tighter leading-[1.1] mb-6">
              Master The <br/> <span className="text-transparent bg-clip-text bg-gradient-to-r from-white via-green-400 to-green-600">Atmosphere.</span>
            </h1>
          </Reveal>
          <Reveal delay={400}>
                  ...
                  <div className="p-8 flex flex-col flex-grow">
                    ...
                    <div className="grid grid-cols-2 gap-3 mt-auto">
                      <button 
                        ...
                        className="py-3 px-4 bg-green-800 hover:bg-green-700 text-white text-sm font-semibold rounded-xl transition-colors flex items-center justify-center gap-2">
                        <Info className="w-4 h-4" /> Get Info
                      </button>
                      <button 
                        onClick={() => setWechatModal(true)}
                        className="py-3 px-4 bg-white text-black hover:bg-green-200 text-sm font-bold rounded-xl transition-colors flex items-center justify-center gap-2 shadow-[0_0_20px_rgba(255,255,255,0.1)] hover:shadow-[0_0_25px_rgba(255,255,255,0.2)]"
                      >
                        <MessageCircle className="w-4 h-4" /> Buy via WeChat
                      </button>
                    </div>
                  </div>
                </div>
            ...
            {features.map((feat, idx) => (
              <Reveal key={idx} delay={idx * 100}>
                <div className="p-6 bg-green-950/50 backdrop-blur-md rounded-2xl border border-white/5 hover:border-white/10 transition-colors">
                  ...
                  <p className="text-sm text-green-500 font-light leading-relaxed">{feat.desc}</p>
                </div>
              </Reveal>
            ))}
          </div>
            ...
            {testimonials.map((test, idx) => (
              <Reveal key={idx} delay={idx * 200}>
                <div className="p-8 bg-gradient-to-br from-green-800/50 to-green-950/50 rounded-2xl border border-white/5 relative">
                  ...
                  <p className="text-lg text-green-300 italic mb-8 font-light leading-relaxed">"{test.quote}"</p>
                  <div>
                    <p className="font-semibold text-white">{test.author}</p>
                    <p className="text-sm text-green-500">{test.role}</p>
                  </div>
                </div>
              </Reveal>
            ))}
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12">
          <Reveal>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
              ...
              {/* Form */}
              <div>
                <h2 className="text-4xl font-bold mb-4">Initialize Contact.</h2>
                <p className="text-green-400 mb-8 font-light">Our engineering and sales team is ready to design your customized atmospheric solution.</p>
                
                <form className="space-y-6" onSubmit={(e) => e.preventDefault()}>
  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
    <div>
      <label className="block text-xs font-semibold text-green-500 uppercase tracking-wider mb-2">First Name</label>
      <input type="text" className="w-full bg-white border border-gray-300 rounded-xl px-4 py-3 text-black focus:outline-none focus:border-emerald-500 transition-colors" placeholder="John" />
    </div>
    <div>
      <label className="block text-xs font-semibold text-green-500 uppercase tracking-wider mb-2">Last Name</label>
      <input type="text" className="w-full bg-white border border-gray-300 rounded-xl px-4 py-3 text-black focus:outline-none focus:border-emerald-500 transition-colors" placeholder="Doe" />
    </div>
  </div>
  <div>
    <label className="block text-xs font-semibold text-green-500 uppercase tracking-wider mb-2">Corporate Email</label>
    <input type="email" className="w-full bg-white border border-gray-300 rounded-xl px-4 py-3 text-black focus:outline-none focus:border-emerald-500 transition-colors" placeholder="john@company.com" />
  </div>
  <div>
    <label className="block text-xs font-semibold text-green-500 uppercase tracking-wider mb-2">Project Requirements</label>
    <textarea
      value={contactMessage}
      onChange={(e) => setContactMessage(e.target.value)}
      className="w-full bg-white border border-gray-300 rounded-lg p-4 text-black focus:outline-none focus:border-emerald-500 transition-colors min-h-[150px]"
      placeholder="Describe your scale and specs..."
    ></textarea>
  </div>
  <button className="w-full py-4 bg-white text-black font-bold rounded-xl hover:bg-green-200 transition-colors flex justify-center items-center gap-2">
    Submit Inquiry <ChevronRight className="w-5 h-5" />
              ...
              {/* Direct Contact Info */}
              <div className="flex flex-col justify-center bg-green-900/30 rounded-3xl p-8 border border-white/5">
                <h3 className="text-xl font-semibold mb-8">Direct Channels</h3>
                <div className="space-y-8">
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-white mb-1">Email Sales</p>
                      <a href="mailto:sales@aethersystems.inc" className="text-green-400 hover:text-emerald-400 transition-colors text-sm font-light">sales@aethersystems.inc</a>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-white mb-1">Global HQ (WhatsApp available)</p>
                      <p className="text-green-400 text-sm font-light">+1 (800) 555-0199</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-white mb-1">WeChat Official Account</p>
                      <button onClick={() => setWechatModal(true)} className="text-[#07C160] hover:text-white transition-colors text-sm font-medium">
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-white mb-1">Manufacturing Facility</p>
                      <p className="text-green-400 text-sm font-light leading-relaxed">
                        900 Industrial Parkway<br/>
                    ...
                    <div className="flex justify-center mt-12 pt-8 border-t border-green-800/50">
        <button
          onClick={() => document.getElementById('products')?.scrollIntoView({ behavior: 'smooth' })}
          className="flex items-center gap-2 px-8 py-3 bg-green-900 hover:bg-green-800 text-white font-medium rounded-full transition-all border border-green-800 hover:border-emerald-500 group shadow-lg"
        >
          <svg 
            className="w-5 h-5 text-emerald-500 group-hover:-translate-y-1 transition-transform" 
            fill="none" 
            stroke="currentColor" 
            viewBox="0 0 24 24"
          >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-green-950/80 backdrop-blur-sm">
          <div className="bg-green-900 border border-green-800 p-8 rounded-2xl max-w-2xl w-full relative animate-in fade-in zoom-in duration-300">
            ...
            <button 
              ...
              className="absolute top-3 right-3 z-50 p-3 bg-green-800 md:bg-transparent rounded-full text-white md:text-green-400 hover:text-white transition-all border border-green-700 md:border-transparent shadow-lg md:shadow-none"
            >
            ...
            <div className="flex flex-col md:flex-row gap-6">
              <div className="w-full md:w-1/2 rounded-xl overflow-hidden bg-green-800 border border-green-700">
                <img src={infoProduct.image} alt={infoProduct.name} className="w-full h-full object-cover opacity-80" />
              </div>
              
              <div className="w-full md:w-1/2 flex flex-col justify-center text-left">
                <h3 className="text-3xl font-bold text-white mb-2">{infoProduct.name}</h3>
                ...
                <button 
                  onClick={() => {
                    // 1. Формируем текст
                    const templateText = `Hello! I'm interested in: ${infoProduct.name}. Could you please provide more details about pricing and specifications?`;
                    
                    // 2. Сохраняем текст для формы
                    setContactMessage(templateText);
                    
                    // 3. Закрываем окно
                    setInfoProduct(null);
                    
                    // 4. Плавно скроллим к форме
                    setTimeout(() => {
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-green-900 border border-green-700/50 rounded-3xl p-8 max-w-sm w-full shadow-2xl animate-in zoom-in-95 slide-in-from-bottom-4 duration-400">
            ...
            <div className="text-center">
              ...
              <p className="text-green-400 text-sm font-light mb-8">Scan the QR code below using another phone to contact our sales team directly.</p>
